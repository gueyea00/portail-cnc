...
const etape1Schema = z.object({
  ...
  qualite: z.string().min(2, "Pr̩cisez votre qualit̩"),
  email: z.string().email("Email invalide"),
  telephone: z.string().min(8, "Num̩ro invalide"),
...
const etape2Schema = z.object({
  ...
  description: z.string().min(20, "Minimum 20 caract̬res"),
});

type Etape1Data = z.infer<typeof etape1Schema>;
type Etape2Data = z.infer<typeof etape2Schema>;

const etapes = ["Identit̩", "Nature de la plainte", "Pi̬ces jointes", "R̩capitulatif"];
...
export default function PlaintePage() {
  ...
  const onSubmit = () => {
    if (!honneur) { toast.error("Veuillez cocher la d̩claration sur l'honneur."); return; }
    setLoading(true);
    setTimeout(() => {
      ...
      setSubmitted(true);
      setLoading(false);
      toast.success("Plainte d̩pos̩e avec succ̬s !");
    }, 2000);
  };
  ...
  if (submitted) {
    return (
      <div>
        <section className="page-hero"><div className="container-page"><h1 className="text-3xl font-bold">Plainte d̩pos̩e</h1></div></section>
        <div className="container-page py-16 text-center">
          <CheckCircle className="w-16 h-16 text-green-500 mx-auto mb-4" />
          <h2 className="text-2xl font-bold text-foreground mb-2">Votre plainte a ̩t̩ enregistr̩e</h2>
          <p className="text-muted-foreground mb-4">Votre num̩ro de r̩f̩rence :</p>
          <p className="text-2xl font-bold text-primary mb-6">{reference}</p>
          <p className="text-sm text-muted-foreground max-w-md mx-auto mb-8">
            Conservez ce num̩ro pour le suivi de votre dossier. Un accus̩ de r̩ception vous sera envoy̩ par email dans un d̩lai de 15 jours ouvrables.
          </p>
          <Button onClick={() => { setSubmitted(false); setStep(0); setData1(null); setData2(null); setFichiers([]); setHonneur(false); }}>
            D̩poser une autre plainte
          </Button>
        </div>
      </div>
    );
      ...
      <section className="page-hero">
        <div className="container-page">
          <h1 className="text-3xl md:text-4xl font-bold flex items-center justify-center md:justify-start gap-4">
            ...
            D̩p̫t de plainte en ligne
          </h1>
          <p className="mt-2 opacity-90 text-lg">Formulaire s̩curis̩ en 4 ̩tapes</p>
        </div>
      </section>
      <Breadcrumb />
      ...
      <div className="container-page py-12 max-w-3xl mx-auto">
        ...
        {step === 0 && (
          <form onSubmit={onStep1} className="space-y-4">
            <h2 className="text-xl font-bold text-foreground mb-4">Identit̩ du plaignant</h2>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div><label className={labelClass}>Pr̩nom *</label><input {...form1.register("prenom")} className={inputClass} />{form1.formState.errors.prenom && <p className={errorClass}>{form1.formState.errors.prenom.message}</p>}</div>
            </div>
            <div><label className={labelClass}>Qualit̩ *</label><input {...form1.register("qualite")} placeholder="Ex: Consommateur, Entreprise..." className={inputClass} />{form1.formState.errors.qualite && <p className={errorClass}>{form1.formState.errors.qualite.message}</p>}</div>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div><label className={labelClass}>T̩l̩phone *</label><input {...form1.register("telephone")} className={inputClass} />{form1.formState.errors.telephone && <p className={errorClass}>{form1.formState.errors.telephone.message}</p>}</div>
            </div>
          ...
          <form onSubmit={onStep2} className="space-y-4">
            ...
            <div><label className={labelClass}>Type de pratique *</label>
              <select {...form2.register("typePratique")} className={inputClass}>
                ...
                <option value="Pratique commerciale d̩loyale">Pratique commerciale d̩loyale</option>
            ...
            <div><label className={labelClass}>Secteur d'activit̩ *</label>
              <select {...form2.register("secteur")} className={inputClass}>
                ...
                <option value="BTP">BTP / Mat̩riaux</option>
                <option value="Sant̩">Sant̩ / Pharmacie</option>
            ...
            <div><label className={labelClass}>Description d̩taill̩e *</label><textarea {...form2.register("description")} rows={5} className={inputClass} />{form2.formState.errors.description && <p className={errorClass}>{form2.formState.errors.description.message}</p>}</div>
            <div className="flex justify-between pt-4">
              <Button type="button" variant="outline" onClick={() => setStep(0)} className="gap-2"><ArrowLeft className="w-4 h-4" /> Pr̩c̩dent</Button>
              <Button type="submit" className="gap-2">Suivant <ArrowRight className="w-4 h-4" /></Button>
            </div>
          </form>
        )}
        ...
        {step === 2 && (
          <div className="space-y-4">
            <h2 className="text-xl font-bold text-foreground mb-4">Pi̬ces jointes</h2>
            <p className="text-sm text-muted-foreground mb-4">Ajoutez les documents justificatifs de votre plainte (contrats, factures, ̩changes de correspondance, etc.)</p>
            <div className="border-2 border-dashed border-border rounded-lg p-8 text-center">
              <Upload className="w-10 h-10 text-muted-foreground mx-auto mb-3" />
              <p className="text-sm text-muted-foreground mb-2">Glissez-d̩posez vos fichiers ici ou</p>
              <Button variant="outline" size="sm" onClick={() => { setFichiers([...fichiers, `document_${fichiers.length + 1}.pdf`]); toast.info("Fichier ajout̩ (simulation)"); }}>
            ...
            <div className="flex justify-between pt-4">
              <Button variant="outline" onClick={() => setStep(1)} className="gap-2"><ArrowLeft className="w-4 h-4" /> Pr̩c̩dent</Button>
        ...
        {step === 3 && data1 && data2 && (
          <div className="space-y-6">
            <h2 className="text-xl font-bold text-foreground mb-4">R̩capitulatif</h2>
            <div className="bg-surface rounded-lg border border-border divide-y divide-border">
              <div className="p-4">
                <h3 className="text-sm font-semibold text-primary mb-2">Identit̩ du plaignant</h3>
                <div className="grid grid-cols-2 gap-2 text-sm">
                  <div><span className="text-muted-foreground">Nom :</span> {data1.nom}</div>
                  <div><span className="text-muted-foreground">Pr̩nom :</span> {data1.prenom}</div>
                  <div><span className="text-muted-foreground">Email :</span> {data1.email}</div>
                  <div><span className="text-muted-foreground">T̩l :</span> {data1.telephone}</div>
                  <div className="col-span-2"><span className="text-muted-foreground">Qualit̩ :</span> {data1.qualite}</div>
              ...
              <div className="p-4">
                <h3 className="text-sm font-semibold text-primary mb-2">Pi̬ces jointes</h3>
                <p className="text-sm">{fichiers.length > 0 ? fichiers.join(", ") : "Aucune pi̬ce jointe"}</p>
              </div>
            </div>

            <label className="flex items-start gap-3 cursor-pointer">
              <input type="checkbox" checked={honneur} onChange={(e) => setHonneur(e.target.checked)} className="mt-1 accent-primary" />
              <span className="text-sm text-foreground">Je d̩clare sur l'honneur que les informations fournies sont exactes et que la pr̩sente plainte est formul̩e de bonne foi.</span>
            </label>

            <div className="flex justify-between pt-4">
              <Button variant="outline" onClick={() => setStep(2)} className="gap-2"><ArrowLeft className="w-4 h-4" /> Pr̩c̩dent</Button>
